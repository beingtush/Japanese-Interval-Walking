...
class OnboardingPrefs(private val context: Context) {
    ...
    companion object {
        ...
        val WEEKLY_WALK_GOAL = intPreferencesKey("weekly_walk_goal")
    }
    ...
    val weeklyWalkGoal: Flow<Int> = context.onboardingDataStore.data
        .catch { exception ->
            if (exception is IOException) {
                Log.e(TAG, "Error reading preferences", exception)
                emit(emptyPreferences())
        ...
        .map { preferences ->
            preferences[WEEKLY_WALK_GOAL] ?: 5
        }
    ...
    suspend fun setWeeklyWalkGoal(goal: Int) {
        try {
            context.onboardingDataStore.edit { preferences ->
                preferences[WEEKLY_WALK_GOAL] = goal
            }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to write weekly walk goal", e)
        }
    }
